...
public class SerializeAndDeserializeBinaryTree {
    ...
    public String serialize(TreeNode root) {
        return new Heap(root).toString();
    }

    // Decodes your encoded data to tree.
    public TreeNode deserialize(String data) {
        return new Heap(data).getTree();
    }

    public static int depthOfBinaryTree(TreeNode node) {
        if (null == node) {
            return 0;
        }

        return Math.max(depthOfBinaryTree(node.left), depthOfBinaryTree(node.right)) + 1;
    }
    ...
    public class Heap {
        ...
        public Heap(TreeNode tree) {
            if (null == tree) {
                return;
            }
            this.data = new Integer[1 << depthOfBinaryTree(tree)];
            this.fillArr(tree);
        }

        public Heap(String serializedTree) {
            if (null == serializedTree) {
                return;
            }
            StringBuilder sb = new StringBuilder(serializedTree);
            sb.deleteCharAt(0);
        ...
        private void fillTree(TreeNode tree, int index) {
            int leftIndex = kthChild(index, 1);
            if (leftIndex < data.length) {
                Integer leftVal = data[leftIndex];
                if (null != leftVal) {
                    TreeNode leftChild = new TreeNode(leftVal);
                    tree.left = leftChild;
                    fillTree(leftChild, leftIndex);
                }
            }
            int rightIndex = kthChild(index, 2);
            if (rightIndex < data.length) {
                Integer rightVal = data[rightIndex];
                if (null != rightVal) {
                    TreeNode rightChild = new TreeNode(rightVal);
                    tree.right = rightChild;
                    fillTree(rightChild, rightIndex);
                }
            }
        }

        private void fillArr(TreeNode tree) {
            if (null == tree) {
                return;
            }
            data[0] = tree.val;
            travel(tree.left, 1, 0);
            travel(tree.right, 2, 0);
        }

        private void travel(TreeNode node, int leftOrRight, int parentIndex) {
            if (null == node) {
                return;
            }

            int curIndex = kthChild(parentIndex, leftOrRight);
            data[curIndex] = node.val;
            travel(node.left, 1, curIndex);
            travel(node.right, 2, curIndex);
        }
